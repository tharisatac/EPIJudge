...
def find_min_max(A: List[int]) -> MinMax:
    ...
    global_min_max = _min_max(A[0], A[1])
    for i in range(2, len(A) - 1, 2):
        local_min_max = _min_max(A[i], A[i + 1])
        global_min_max = MinMax(
            min(global_min_max.smallest, local_min_max.smallest),
            max(global_min_max.largest, local_min_max.largest),
        )

    # Compare the last value if the len of the array is not even.
    if len(A) % 2:
        global_min_max = MinMax(
            min(A[-1], global_min_max.smallest),
            max(A[-1], global_min_max.largest),
        )
